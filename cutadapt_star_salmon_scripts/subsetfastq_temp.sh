...
awk -v k=20000000 'BEGIN{srand(systime() + PROCINFO["pid"]);}{s=x++<k?x1:int(rand()*x);if(s<k)R[s]=$0}END{for(i in R)print R[i]}' | \
awk -F "\t" '{print $1"\n"$3"\n"$5"\n"$7 > "MM142sub_S1_L001_R1_001.fastq" ; print $2"\n"$4"\n"$6"\n"$8 > "MM142sub_S1_L001_R2_001.fastq"}'

paste MM143_S1_L001_R1_001.fastq MM143_S1_L001_R2_001.fastq | awk '{ printf("%s",$0); n++; if(n%4==0) { printf("\n");} else { printf("\t");} }' | \
awk -v k=20000000 'BEGIN{srand(systime() + PROCINFO["pid"]);}{s=x++<k?x1:int(rand()*x);if(s<k)R[s]=$0}END{for(i in R)print R[i]}' | \
awk -F "\t" '{print $1"\n"$3"\n"$5"\n"$7 > "MM143sub_S1_L001_R1_001.fastq" ; print $2"\n"$4"\n"$6"\n"$8 > "MM143sub_S1_L001_R2_001.fastq"}'

paste MM144_S1_L001_R1_001.fastq MM144_S1_L001_R2_001.fastq | awk '{ printf("%s",$0); n++; if(n%4==0) { printf("\n");} else { printf("\t");} }' | \
awk -v k=20000000 'BEGIN{srand(systime() + PROCINFO["pid"]);}{s=x++<k?x1:int(rand()*x);if(s<k)R[s]=$0}END{for(i in R)print R[i]}' | \
awk -F "\t" '{print $1"\n"$3"\n"$5"\n"$7 > "MM144sub_S1_L001_R1_001.fastq" ; print $2"\n"$4"\n"$6"\n"$8 > "MM144sub_S1_L001_R2_001.fastq"}'

paste MM145_S1_L001_R1_001.fastq MM145_S1_L001_R2_001.fastq | awk '{ printf("%s",$0); n++; if(n%4==0) { printf("\n");} else { printf("\t");} }' | \
awk -v k=20000000 'BEGIN{srand(systime() + PROCINFO["pid"]);}{s=x++<k?x1:int(rand()*x);if(s<k)R[s]=$0}END{for(i in R)print R[i]}' | \
awk -F "\t" '{print $1"\n"$3"\n"$5"\n"$7 > "MM145sub_S1_L001_R1_001.fastq" ; print $2"\n"$4"\n"$6"\n"$8 > "MM145sub_S1_L001_R2_001.fastq"}'

paste MM135_S1_L001_R1_001.fastq MM135_S1_L001_R2_001.fastq | awk '{ printf("%s",$0); n++; if(n%4==0) { printf("\n");} else { printf("\t");} }' | \
awk -v k=20000000 'BEGIN{srand(systime() + PROCINFO["pid"]);}{s=x++<k?x1:int(rand()*x);if(s<k)R[s]=$0}END{for(i in R)print R[i]}' | \
awk -F "\t" '{print $1"\n"$3"\n"$5"\n"$7 > "MM135sub_S1_L001_R1_001.fastq" ; print $2"\n"$4"\n"$6"\n"$8 > "MM135sub_S1_L001_R2_001.fastq"}'

paste K10R_S1_L001_R1_001.fastq K10R_S1_L001_R2_001.fastq | awk '{ printf("%s",$0); n++; if(n%4==0) { printf("\n");} else { printf("\t");} }' | \
awk -v k=20000000 'BEGIN{srand(systime() + PROCINFO["pid"]);}{s=x++<k?x1:int(rand()*x);if(s<k)R[s]=$0}END{for(i in R)print R[i]}' | \
awk -F "\t" '{print $1"\n"$3"\n"$5"\n"$7 > "K10Rsub_S1_L001_R1_001.fastq" ; print $2"\n"$4"\n"$6"\n"$8 > "K10Rsub_S1_L001_R2_001.fastq"}'
