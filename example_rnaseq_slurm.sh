...
#SBATCH --job-name=projectX_rnaseq_align
...
#SBATCH --output=/homes/daniel.thomson/bioinfoCore/projectX/align.log
...
# Run fastqc + STAR alignment + quantification
...
AdapterRemoval=/homes/jimmy.breen/bin/AdapterRemoval 	##DT this isn't there
...
${AdapterRemoval} --file1 $i --gzip --output1 ${base}/1_AdapterRemoval/`basename $i _1.fastq.gz`_tr1.fastq.gz

# Run fastqc

mkdir -p ${base}/1a_trimQC

${fastqc} -o ${base}/1a_trimQC ${base}/1_AdapterRemoval/`basename $i _1.fastq.gz`_tr1.fastq.gz
...
--readFilesIn ${base}/1_AdapterRemoval/`basename $i _1.fastq.gz`_tr1.fastq.gz \
...
--outFileNamePrefix ${base}/2_star/`basename $i _1.fastq.gz`_${genome}_ \
