#!/bin/bash
#SBATCH --job-name=basespace_delete_run
#SBATCH --output=bs_delete_run.log
...
bs -c SAGC list run --older-than=1y

for projectID in $(bs -c SAGC list run --older-than=1y --terse)
do
        ...
        bs -c SAGC project get -i ${projectID}
        bs -c SAGC delete run -i ${projectID} --preserve-metadata
done

bs -c SAGC list projects --older-than=5y
